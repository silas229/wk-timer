...
import { GET as shareRoundGET, POST as shareRoundPOST, OPTIONS as shareRoundOPTIONS } from '@/app/api/share-round/route';
import { GET as oembedGET, OPTIONS as oembedOPTIONS } from '@/app/api/oembed/route';
...
describe('API Integration Tests', () => {
  ...
  describe('Cross-API integration: Share Round → oEmbed', () => {
    it('should successfully create a share and generate oEmbed for it', async () => {
      ...
      const shareRequest = new NextRequest('http://localhost:3000/api/share-round', {
        method: 'POST',
  ...
  describe('Data validation edge cases', () => {
    it('should handle extremely large lap times', async () => {
      const roundDataWithLargeTimes = {
        ...
        laps: [
          { lapNumber: 1, time: Number.MAX_SAFE_INTEGER / 2, timestamp: '2024-01-01T12:01:00Z' },
        ],
    ...
    it('should handle special characters in team name and description', async () => {
      ...
      const description = 'Description with <script>alert("test")</script> and "quotes"';
      ...
      expect(getData.description).toBe('Description with <script>alert("test")</script> and "quotes"');
    });
